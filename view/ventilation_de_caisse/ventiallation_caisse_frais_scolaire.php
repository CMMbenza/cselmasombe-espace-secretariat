...
/* Date FR pour affichage utilisateur */
...
if ($startDate !== '') {
  $wherePaiement[] = 'DATE(p.dateCreated) >= ?'; $bindPaiement .= 's'; $argsPaiement[] = $startDate;
  $whereDepense[]  = 'd.dateCreaty >= ?';          $bindDepense  .= 's'; $argsDepense[]  = $startDate;
}
if ($endDate !== '') {
  $wherePaiement[] = 'DATE(p.dateCreated) <= ?'; $bindPaiement .= 's'; $argsPaiement[] = $endDate;
  $whereDepense[]  = 'd.dateCreaty <= ?';          $bindDepense  .= 's'; $argsDepense[]  = $endDate;
}

/* Dépenses scolaires : filtre par mot-clé "scol" */
$whereDepense[] = "(LOWER(d.reference) LIKE '%scol%' OR LOWER(d.description) LIKE '%scol%')";

/* Requêtes agrégées PAR JOUR (Utilisation de DATE(p.dateCreated)) */
$sqlPaiement = "SELECT DATE(p.dateCreated) AS d, COALESCE(SUM(p.montantPayer),0) AS entre
                FROM paiement p
                ".(count($wherePaiement) ? "WHERE ".implode(' AND ', $wherePaiement) : "")."
                GROUP BY DATE(p.dateCreated) ORDER BY DATE(p.dateCreated) ASC";
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title d-flex justify-content-between">
                    ...
                    <a href="ventiallation_caisse_frais_connexe.php" class="btn btn-dark">Ventiallation caisse frais
                        connexe</a>
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <a class="text-decoration-underline fw-semibold"
                                        href="detail_ventillation.php?type=scolaire&date=<?= urlencode($d) ?>"><?= fmt($e) ?>
                                        $</a>
                                </td>
                                <td class="text-end">
                                    <a class="text-decoration-underline fw-semibold"
                                        href="detail_ventillation.php?type=scolaire&date=<?= urlencode($d) ?>"><?= fmt($x) ?>
                                        $</a>
                                </td>
