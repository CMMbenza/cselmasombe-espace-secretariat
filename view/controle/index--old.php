...
function build_grid(mysqli $con, array $m): array {
  ...
  $payeScol   = (float)$m['montantDejaPayerScol']; // cumuls réels
  ...
  // Divers "référence" injectés dans la 1ère tranche
  $diversRef = (!empty($eleves)) ? fetch_divers_ref($con, $id, $annee) : 0.0;

  // Déterminer la 1ère tranche existante
  $nums = array_keys($apayerByTranche);
  $trancheOneKey = 1;
  if (!empty($nums)) {
    $tmp = array_map('intval', $nums);
    sort($tmp);
    $trancheOneKey = in_array(1,$tmp,true) ? 1 : (int)$tmp[0];
  }

  // Ajouter les divers ref à la tranche 1
  if (!isset($apayerByTranche[$trancheOneKey])) $apayerByTranche[$trancheOneKey] = 0.0;
  $apayerByTranche[$trancheOneKey] += (float)$diversRef;

  // Re-ordonner et totaux
  $nums = array_keys($apayerByTranche);
  ...
  // Ventilation séquentielle du payé annuel
  $paidLeft = $totalPaye;
  $paidBy = []; $resteBy = [];
  foreach ($nums as $n) {
  ...
  return [
    ...
    'apayerScolOnly'          => $apayerScolOnly,
    'diversRef'               => $diversRef,
    'trancheOneKey'           => $trancheOneKey,
    'paidBy'                  => $paidBy,
    'resteBy'                 => $resteBy,
    'totalAPayer'             => $totalAPayer,
    'totalPaye'               => $totalPaye,
    'totalReste'              => $totalReste,
    'surplus'                 => max($paidLeft, 0.0),
  ];
}
